...
class ArcPlanner:
    ...
    def build_spine(self, world: WorldConfig, target_chapters: int, brief=None, bus=None) -> NarrativeSpine:
        ...
        sys = ("너는 웹소설 아크 설계자다. '엔딩을 먼저 확정'하고 거기서 역순(backward)으로 아크를 설계하라. "
               "**철칙: 갈등의 종류·규모·무대는 오직 이 작품의 장르·톤·전제·세계규칙이 결정한다.** "
               "주어진 세계에 실제로 없는 장치를 임의로 끌어들이지 마라 — 회귀·빙의·각성·시스템·절대권능·흑막 세력·"
               "세계정복·최종결전 같은 파워판타지 클리셰를, 전제에 정말로 존재하지 않는 한 절대 넣지 마라. "
               "잔잔한 일상·학원·로맨스·드라마라면 갈등은 전투가 아니라 관계·오해·감정·선택·진로·성장처럼 그 장르다운 "
               "일상의 결이어야 한다(적·세력·결전을 만들지 마라). "
               "각 에피소드의 절정(climax)은 '그 장르의 카타르시스'다 — 액션물=전투의 쾌감, 로맨스=감정의 폭발, "
               "일상물=관계의 전환·작은 용기, 미스터리=반전. **전투를 기본값으로 삼지 마라.** "
               "전제·시놉시스·세계규칙·작가 설계에 담긴 '발단부터 결말까지의 흐름'을 그대로 살려 설계하라. "
