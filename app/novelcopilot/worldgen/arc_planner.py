...
class ArcPlanner:
    ...
    def beat_for_episode(self, world: WorldConfig, arc: Arc, episode: Episode, chapter: int,
        ...
        # 발단(도입부)을 1~3화 '미니아크'로 페이싱(연구: 전독시는 구체적 일상 grounding 후 전환 극화 / 한국 관습 = 3화에 setup→전개→전환→귀결 압축,
        # 한 줄 라벨·인포덤프·즉시 액션 금물). chapter==1 단일 압축이 사용자 지적('한 줄짜리')의 원인이라 단계 분산.
        is_opening = (chapter <= 3)
        if chapter == 1:
            hook = ("이번은 작품의 '도입부 1/3화'다. 본격 전개(레벨업·공략·각성 후 활약 등 작품의 핵심 엔진)로 직행하지 마라. "
                    "먼저 주인공이 '누구'인지를 한 줄 라벨('10년 게임한 고인물이다' 식)이 아니라 *구체적인 장면*으로 보여줘라 — "
                    "직업·하루의 결·처지·결핍·관계가 독자 눈앞에 그려지게(예: 전독시가 김독자를 '지하철 통근하며 웹소설 읽는 미노소프트 계약직, "
                    "그 소설의 유일한 독자'로 구체화한 식의 손에 잡히는 디테일). 그 일상 위로 전제의 전환이 *시작되는 조짐/첫 균열*을 깔고 끝에 훅을 남겨라. "
                    "전환을 1화에서 다 끝내려 하지 마라.")
        elif chapter == 2:
            hook = ("이번은 작품의 '도입부 2/3화'다. 1화에서 깔린 조짐이 현실이 되는 전환의 순간을 장면으로 극화하라. "
                    "세계의 규칙은 설명(인포덤프)이 아니라 주인공의 구체적 행동·시점으로 흘려 보여주고, 첫 실질적 이해관계·대가가 생기게 하라. "
                    "아직 주인공이 '강자'로 마음껏 활약하진 않는다 — 막 변화에 적응하는 단계다.")
        elif chapter == 3:
            hook = ("이번은 작품의 '도입부 3/3화 — 매듭'이다. 도입부 미니아크를 매듭지어라 — 여기서 처음으로 작은 전환점/사이다"
                    "(주인공만의 무언가가 처음으로 통하는 순간)를 주고, 본격 엔진으로 진입하는 발판을 놓아라. "
                    "이 시점이면 독자는 이 인물이 누구이고, 무엇이 변했고, 왜 중요한지 충분히 알고 있어야 한다.")
        elif is_finale:
