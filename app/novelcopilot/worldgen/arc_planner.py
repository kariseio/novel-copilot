...
class ArcPlanner:
    ...
    def beat_for_episode(self, world: WorldConfig, arc: Arc, episode: Episode, chapter: int,
        ...
        if chapter == 1:
            ...
            hook = ("아직 작품의 '도입부'(첫 에피소드) 안이다. 1화에서 연 도입을 이어받아 전환의 여파를 겪고, 세계의 규칙을 주인공의 행동으로 발견하며 "
                    "첫 이해관계·대가가 쌓이게 하라(직전 화 사건 반복 금지, 한 단계 전진). 본격 엔진의 전면 가동·결정적 사이다는 도입부를 매듭지을 때.")
        elif is_finale:
            hook = ("이번 회차가 에피소드 절정(finale): 아래 climax 를 이번 회차에서 터뜨려라(끝맺음 방식은 작품 문체 정책을 따름)."
                    + (" 도입부의 매듭이기도 하니, 주인공만의 강점이 처음으로 결정적으로 통하는 첫 전환점이 되게." if in_opening else ""))
        else:
