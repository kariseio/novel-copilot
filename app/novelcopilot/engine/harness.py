...
# 외국어 토큰 누출 백스톱: gpt-5.5 등이 한국어 prose 중 외국어 단어(예 'удар')를 흘림. 키릴·그리스·히브리·아랍·
# 데바나가리·태국 — 한국 웹소설 prose 에 정상 출현 0(한자 CJK·라틴 영문은 제외 — 무협 한자·게임명 등 정상). run 단위 검출.
_FOREIGN_RE = re.compile(r"[Ѐ-ӿͰ-Ͽ֐-׿؀-ۿऀ-ॿ฀-๿]+")
...
_CRAFT_PROGRESS = ("\n\n[전개 압력] 이 회차는 직전 장면·상황을 같은 자리에서 되풀이하지 말고 새 국면으로 전진시켜라 — "
                   "이 작품의 전제·직전 상황이 변화를 추적하는 축(장소·관계·정보·정황 등 작품이 정한 것) 중 최소 하나가 회차 안에서 실제로 바뀌어야 한다. "
                   "직전과 같은 장면·같은 묘사·같은 회상 대사를 그대로 반복하지 말고, 한 장면을 길게 늘여 분량을 채우지 말고 사건이 전진해 자연히 길어지게 하라.")

# 장면형 스타일 앵커(실험) — 블랭킷 지시의 '과적용(진자)'을 피해, 비트 기능(chapter_function, 결정론)에 *선택*을 묶는다.
# 전역으로 최대화되는 지시가 아니라 '이 장면의 결'만 짧게 — 필요한 장면에만 맞게.
_SCENE_ANCHORS = {
    "escalation": "[이 장면의 결: 속도] 짧은 호흡으로 몰아쳐라 — 문장을 짧게 끊고, 짧은 동작·대사를 부딪치며 이 장면의 긴장을 한 칸씩 올려라.",
    "payoff": "[이 장면의 결: 터뜨림] 헤지('~듯한/~같은') 없이 확정 서술로 타격감을 줘라 — 쌓아온 것의 무게가 한 방에 드러나게.",
...
class ChapterGenerator:
    ...
    _HOOKS = {   # 회차 끝맺음 정책(StyleSpec.ending_hook 데이터 주도). 작법 '명칭'(절단신공/훅 등)은 모델 입력 금지
        "cliffhanger": ("\n\n[이 장면은 회차의 마지막] 깔끔히 닫지 말고, 이 회차가 쌓아온 긴장·물음에서 자연스럽게 자라난 "
                        "미결의 순간에서 끊어 다음 화가 궁금해지게 하라(마지막 한두 줄에서 멈춰라)."),   # 은어 누출('절단.' 단독행) 소스 제거
    ...
    def _draft(self, board, scene, prev_scenes, last=False, closing=False, recent_tails=None,
        ...
        if last:
            ...
            if hook and not closing and recent_tails:
                ...
                hook += ("\n(최근 회차들의 끝:\n" + tails +
                         "\n— 위에 나열된 최근 끝맺음과 같은 수법·같은 결의 마무리를 반복하지 말고, "
                         "이 회차가 실제로 쌓은 것에서 자라난 다른 결의 끝으로 끊어라.)")
    ...
    def _fix_foreign(self, chapter_text: str, runs: list[str]) -> str:
        """외국어 토큰 누출(코드스위칭, gpt-5.5 등) 국소 교정 — 비한국어 외국문자 단어를 한국어로 치환(find/replace, 재생성 금지)."""
        try:
            res = self.provider.chat_json(
                [{"role": "system", "content":
                  "한국 웹소설 본문에 비한국어 외국어 단어가 잘못 섞였다: " + ", ".join(f"'{w}'" for w in runs[:12]) + ". "
                  "각 외국어 단어(및 그에 붙은 조사 포함)를 그 문맥에 가장 자연스러운 한국어로 바꿔라. 그 단어들 외에는 한 글자도 바꾸지 마라(인명·사건·수치 불변). "
                  '치환 쌍으로만. JSON: {"fixes":[{"find":"본문에 실제로 있는 외국어 포함 구절","replace":"한국어"}]}'},
                 {"role": "user", "content": chapter_text}],
                temperature=0.0, max_tokens=1500)
            n = 0
            for fx in (res.get("fixes") or [])[:30]:
                f, r = (fx.get("find") or ""), (fx.get("replace") or "")
                if f and r and r != f and chapter_text.count(f) >= 1:
                    chapter_text = chapter_text.replace(f, r); n += 1
            self.bus.emit("quality", "foreign_fixes", applied=n, runs=runs[:8])
        except Exception:
            pass
        return chapter_text
    ...
    def generate(self, ch_no, beat, ontology, rag, wiki, directives=None,
        ...
        _foreign = list(dict.fromkeys(_FOREIGN_RE.findall(chapter_text)))   # 비한국어 외국문자(키릴·그리스·아랍 등) 누출 — gpt-5.5 코드스위칭 백스톱
        if _foreign:
            _t = self.provider.usage.chat_tokens
            chapter_text = self._fix_foreign(chapter_text, _foreign[:20])
            draft_ctx["corrections"].append(f"fix_foreign({','.join(_foreign[:3])})")   # D-8
            _track("quality", _t)
