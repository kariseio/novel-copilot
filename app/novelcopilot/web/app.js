...
function _onlyView(id){ ["view-home","view-create","view-project","view-viewer","view-skills"].forEach(v=>$("#"+v).classList.toggle("hidden", v!==id)); }
...
function renderReader(){
  ...
  body.innerHTML = `<h3 class="ch-title">${c.chapter}화 · ${esc(c.title)} ${badge}${reviseBtn}</h3>`+
    ...
    rec+readerBlock+violBlock+claimBlock+
...
function renderViewer(){
  ...
  $("#v-body").innerHTML = mdToHtml(c.text||"");
...
async function resumeGenerationIfRunning(pid){
  ...
  if(st.status!=="running") return;   // 완료분은 이미 본문에 반영돼 정상 렌더 — 라이브만 재연결
  STATE.generating = true; _genDone = false;
  $("#gen-btn").disabled = true;
